...
| 6 | **Lift `useViewState` to `home.tsx`** (flagged). Export both `displayView` and *legacy* `activeView` so children keep compiling. | 🔶 Medium | Smoke test tab navigation; run full test suite. |
